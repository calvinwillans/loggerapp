...
// Stages that start a new row whenever the button colour changes.
...
function renderStage() {
  ...
  // Incident and Location are laid out in bands: one row (or more, if it
  // overflows) per colour, so a colour change always starts a new line and
  // the eye can follow an area straight across. Callsign and Department flow
  // as one continuous grid.
  const banded = BANDED_STAGES.indexOf(stage) !== -1;
  const cells = [];
