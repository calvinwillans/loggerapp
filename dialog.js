...
// Stages that start a new row whenever the button colour changes. Inside a
// folder (People, FOH and so on) buttons are always banded, whatever the
// stage, since that's where a colour grouping is most useful.
const BANDED_STAGES = ["incident", "location"];
...
function renderStage() {
  ...
  // Banded screens start a new row at each colour change, so the eye can
  // follow a group straight across. The top-level Callsign and Department
  // screens flow as one continuous grid instead.
  const banded = BANDED_STAGES.indexOf(stage) !== -1 || (paths[stage] && paths[stage].length > 0);
  let cells = [];
  ...
  for (const f of folders) {
    ...
    t.appendChild(count);
    cells.push({ colour: bandKey(f.colour), row: f.row, el: t });
  }

  const labels = shortLabels(items);
  const compact = folders.length === 0 && items.length > 0 && labels.every((l) => l.length <= 4);

  items.forEach((item, idx) => {
    const t = tile(labels[idx], compact ? "" : item.code, item.colour, () => pick(stage, item));
    t.title = item.label + (item.code ? "  (" + item.code + ")" : "");
    if (sel[stage] === item) t.classList.add("chosen");
    cells.push({ colour: bandKey(item.colour), row: item.row, el: t });
  });

  // Row numbers from Inputs column G win over colour: buttons sharing a
  // number sit on one row, and the rows come out in numerical order.
  const numbered = banded && cells.some((c) => c.row !== null);
  if (numbered) {
    cells = cells
      .map((c, i) => ({ c, i }))
  ...
  for (const cell of cells) {
    if (band === null || (banded && keyOf(cell) !== bandColour)) {
      band = document.createElement("div");
      band.className = "band" + (compact ? " compact" : "");
      grid.appendChild(band);
      bandColour = keyOf(cell);
    }
    band.appendChild(cell.el);
  }
}

/** Buttons are banded by colour, so this is what counts as "the same colour". */
function bandKey(colour) {
  return (colour || "").trim().toLowerCase() || "default";
}
