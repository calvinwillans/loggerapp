...
function renderStage() {
  ...
  // Buttons are laid out in bands: one row (or more, if it overflows) per
  // colour, so a colour change always starts a new line and the eye can
  // follow a department or area straight across.
  const cells = [];
  ...
  for (const cell of cells) {
    if (band === null || cell.colour !== bandColour) {
